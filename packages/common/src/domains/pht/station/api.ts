...
export async function getAPIStations(options?: RequestRecord<Station>) : Promise<CollectionResourceResponse<Station>> {
    const response = await useAPI(APIType.DEFAULT).get('stations' + formatRequestRecord(options));
